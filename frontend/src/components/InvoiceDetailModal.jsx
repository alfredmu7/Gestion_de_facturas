...
export const InvoiceDetailModal = ({ invoice, onClose, onInvoiceUpdated }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      // 1. Enviar aclaración o solicitud de ajuste al servidor (Agente 3)
      const response = await resolveInvoiceReview(currentInvoice.id, messageText, newHistory);
      
      // Axios coloca el JSON del backend en response.data
      const resData = response.data || response;
      const resolution = resData.resolution || {};
      const updatedData = resData.updatedInvoice || resData.data;

      const newStatus = resolution.nuevo_estado_auditoria || resData.nuevo_estado_auditoria || currentStatus || 'APROBADA';
      ...
      setChatHistory([
        ...
        { 
          sender: 'agent', 
          text: resolution.respuesta_usuario || resolution.explicacion_resolucion || 'Solicitud procesada correctamente.' 
        }
      ]);
